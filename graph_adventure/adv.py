...
def dft(starting_vertex, graph):

    stack = Stack()
    visited = set()
    path = []
    traversalPath = []
    directions_for_cache = []

    stack.push(starting_vertex)

    perms = [''.join(p) for p in permutations('nsew')]
    ...
    while stack.size() > 0:
        ...
        if vertex not in visited:
            ...
            directions = perms[random_num]
            directions_for_cache.append(directions)

            for direction in directions:
                if direction in graph[vertex][1]:
                    stack.push(graph[vertex][1][direction])
    ...
    cache[len(traversalPath)] = directions_for_cache
    ...
    while queue.size() > 0:
        ...
        if vertex not in visited:
            ...
            for direction in directions:
                if direction in graph[vertex][1]:
                    new_path = list(path)
                    new_path.append(graph[vertex][1][direction])
                    queue.enqueue(new_path)


def turn_path_into_directions(path, graph):
    directions = "nsew"
    directions_list = []

    for i in range(len(path) - 1):
        vertex = path[i]
        # Check which direction goes to the next value in the path
        for direction in directions:
            if direction in graph[vertex][1]:
                if graph[vertex][1][direction] == path[i + 1]:
